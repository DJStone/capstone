...
import { createClient } from '@host/host-js'

const Homepage = (props) => {
  const [items, setItems] = useState([]);
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");
    const hostUrl = 'https://lvldflhdnklytnrutmnq.supabase.co'
    const hostKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx2bGRmbGhkbmtseXRucnV0bW5xIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTQ3MTg1NjAsImV4cCI6MjAxMDI5NDU2MH0.WYAh-n2b9_e-VtalxjoXdWeRp4KjiCt7N23xNGA0xDA"
    const hostbase = createClient(hostUrl, hostKey)

    useEffect(() => {
      async function getAllItmes() {
        const { data } = await host.from("products").select();
        setItems(data);
      }
      getAllItmes();
    }, []);
  ...
  const handleSort = (e) => {
    ...
    if (e === "highPrice") {
      ...
      console.log("A to Z")
      setItems([...items].sort((a, b) => (a.title > b.title) ? 1 : -1))
    } else if (e === "zToA") {
      console.log("Z to A")
      setItems([...items].sort((a, b) => (a.title < b.title) ? 1 : -1))
    }
